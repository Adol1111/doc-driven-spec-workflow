...
- MUST use `task-spec-execution` only after the current concrete task is chosen and the user is ready to write or revise that task's `spec.md`, optionally `plan.md`, and then move toward implementation.
...
Use `task-spec-execution` when the current concrete task already exists or has just been chosen.
...
- "We know the current task and need to execute it safely." -> `task-spec-execution`
...
Do not use `task-spec-execution` as the primary place to invent milestone structure.
