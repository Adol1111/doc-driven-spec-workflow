...
如果你想使用完整的分阶段 workflow，请安装这个仓库里的三个 skill 目录：

- `skills/docs-delivery-workflow`
- `skills/milestone-planning`
- `skills/doc-driven-spec-workflow`

如果你只需要 current-task execution 阶段，也可以只安装 `skills/doc-driven-spec-workflow`。

安装后重启 agent，让它发现新 skills。
...
2. 需要时，用 `brainstorming` 或等价澄清流程明确模糊意图。
3. 当 roadmap shape 还不清楚时，用 `milestone-planning` 决定 milestone、module 和 task 结构。
4. 在 `docs/tasks/` 下选定当前 concrete task。
5. 用 `doc-driven-spec-workflow` 编写或更新 task-local `spec.md`。
6. 停下来等待用户批准，不直接进入实现。
7. 只有任务确实复杂时才创建 `plan.md`。
8. 运行 readiness checkpoint，并用 branch 或 worktree 隔离实现工作。
9. 实现、验证、更新 docs/status，并处理 branch closing。
...
如果你只需要某一个阶段，也可以只安装对应 skill。

也可以直接从各自的 skill 路径安装：
